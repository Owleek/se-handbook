...
`}
      ...
      <Note title='Декларации'>
        <NoteItem>
          <p>
            Часто в коде приходится работать с внешними скриптами, и ts нужно
            понимать тип этих внешних функций, констант или объектов
          </p>
          <p>
            По этому в проекте к обычным js-модулям создаются файлы описывающие
            функционал js модуля, somemodule.js, somemodule.d.ts, они могут быть
            в папке @types
          </p>
          <p>
            Так же нередко приходится объяснять ts c каким импортом мы работаем,
            какого типа то что мы откуда то импортируем
          </p>
          <pre>
            {`
Если TypeScript встретит импорт файла .scss, мы говорим не считай его неизвестным модулем. Вот его TypeScript-интерфейс
...
тогда ts не будет ругаться на импорт


либо если это обычный css

то просто: declare module "*.css"
и ничего более.
Тут мы говорим что считай что такие модули существуют и тебе не нужно считать его отсутствующим»
`}
          </pre>
          <p>
            В проекте часто бывают global.d.ts - в котором содержаться разные
            описания чего либо.
          </p>
          <p>
            Эти описания потом импортируют различные части кода для работы с
            неизвестной ts функции или сущности
          </p>
          <p>
            В файле *.d.ts нельзя написать реализацию кода - будет ошибка, там
            может быть только описание
          </p>
        </NoteItem>
      </Note>
    </>
