  ...
  // по этому 
    super.method() // мы не получим ближайщий нам прототип Second.prototype.method, тут опять же будет синтаксическая ошибка
        ...
        <p>
          ...
          была создана, т.е функция пустышка не должна иметь своего контекста,
          т.е у нее не должно быть ничего, ни this, ни arguments, ни new.target
          ни свойства prototype, поскольку ее главная задача использовать весь
          внешний контекст, и просто выполнить простую операцию, по этому она не
          может быть использована как полноценная функция для каких либо других
          задач
        </p>
      </NoteItem>
    </Note>
